...
TAG_PREFIX="inquerydata/inquery-trino-gateway:latest"
...
for arch in "${ARCHITECTURES[@]}"; do
    ...
    if [[ "$arch" != "ppc64le" ]]; then
        test_container "${TAG_PREFIX}" "linux/$arch" "${SCRIPT_DIR}/docker-compose.yml" "inquery-trino-gateway" "postgres"
    fi
    docker image inspect -f '🚀 Built {{.RepoTags}} {{.Id}}' "${TAG_PREFIX}"
done
